...
if [ "$UI_MODE" = "proxy" ]; then
  ...
  UI_HTTP_BLOCK="root ${UI_DIR}; index index.html; location /assets/ { try_files \$uri =404; add_header Cache-Control 'no-store'; } location = /registerSW.js { default_type application/javascript; return 204; } location = /sw.js { return 404; } location = /manifest.webmanifest { try_files \$uri =404; add_header Cache-Control 'no-store'; } location / { try_files \$uri /index.html; add_header Cache-Control 'no-store'; }"
  UI_HTTPS_BLOCK="root ${UI_DIR}; index index.html; location /assets/ { try_files \$uri =404; add_header Cache-Control 'no-store'; } location = /registerSW.js { default_type application/javascript; return 204; } location = /sw.js { return 404; } location = /manifest.webmanifest { try_files \$uri =404; add_header Cache-Control 'no-store'; } location / { try_files \$uri /index.html; add_header Cache-Control 'no-store'; }"
fi
